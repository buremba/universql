...
def tt(max_iteration):
    ...
    while True:
        measure_all(cursor, """
        CREATE TEMP TABLE IF NOT EXISTS test45432089222222521 (col1 varchar);

            SELECT * FROM test45432089222222521;
        """)
 #        measure_all(cursor, """
 #         CREATE OR REPLACE TEMPORARY TABLE ICEBERG_TESTS.public.stg_device_metadata (
 #     device_id VARCHAR,
 #     device_name VARCHAR,
 #     device_type VARCHAR,
 ...
 # )
 #        """)
        # measure_all(cursor, "select * from ICEBERG_TESTS.public.\"ttt\" group by all")
