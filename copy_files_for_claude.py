...
# CONFIGURATION
GITIGNORE_FILE = Path('.gitignore') # optional, set to None if not needed
SOURCE_DIRECTORY = Path('../universql')  # Set your source directory
TARGET_DIRECTORY = Path('../universql_files_for_claude')  # Set your target directory
...
class IgnoreParser:
    def __init__(self, gitignore_file: Optional[Path] = None):
            ...
            with open(gitignore_file, 'r') as f:
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        pattern = self._get_ignore_pattern(line)
        ...
        if 'ADDITIONAL_IGNORE' in globals():
            for additional_ignore_item in ADDITIONAL_IGNORE:
                pattern = self._get_ignore_pattern(additional_ignore_item)
                self.ignore_patterns.append(re.compile(pattern))
            print(f"Loaded additional items to ignore defined in ADDITIONAL_IGNORE")

    def _get_ignore_pattern(self, text):
        stripped_text = text.strip()
        pattern = stripped_text.replace('.', r'\.')
...
def copy_specific_files(source_dir: Path = SOURCE_DIRECTORY, 
    ...
    gitignore = IgnoreParser(source_dir / '.gitignore')
    files_copied = 0
    files_ignored = 0
    file_mappings = []
